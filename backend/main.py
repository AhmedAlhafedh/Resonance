...
async def evaluate_quiz(request: EvaluateQuizRequest):
    ...
    if total_questions == 0:
        response = EvaluateQuizResponse(confidence=0, correct_answers=0, total_questions=0, questions=[])
        logger.info(f"FINAL BACKEND RESPONSE: {response.json() if hasattr(response, 'json') else '{}'}")
        return response
    ...
    logger.info(f"FINAL BACKEND RESPONSE: {response.json()}")
