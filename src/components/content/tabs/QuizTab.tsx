        ...
        return (
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="animate-fade-in">
                ...
                <div className="glass-card p-8 text-center mb-4">
                    ...
                    <h3 className="text-2xl font-bold text-text-primary mb-1">{score}/{questions.length}</h3>
                    <p className="text-sm text-text-muted mb-3">{pct}% correct</p>
                    <div className="h-2 bg-surface-4 rounded-full overflow-hidden mb-4">
                        <motion.div
                            className={`h-full rounded-full ${pct >= 80 ? 'bg-emerald-400' : pct >= 60 ? 'bg-amber-400' : 'bg-rose-400'}`}
                            initial={{ width: 0 }}
                            animate={{ width: `${pct}%` }}
                    ...
                    <p className="text-sm text-text-secondary">
                        {pct >= 80 ? '🎉 Excellent work!' : pct >= 60 ? '👍 Good effort, review the misses.' : '📖 Consider re-reading the lecture.'}
                    </p>
                </div>
